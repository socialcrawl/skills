...
- `app_id` is the Google Play package name (e.g. `com.spotify.music`).
- Reviews batch in 150s (max 600).
- Both app stores (Google Play and Apple App Store) share the same 8 resources and the same unified App object with a `store` discriminator (`"google_play"` vs `"app_store"`). Google-only fields: `installs`, `genres`, developer email/address.
...
Search the Google Play listings database (paginated). This is the one paginated resource on the platform, with a real `total` and `next_cursor`.
...
List Google Play app categories. No parameters.
...
List supported Google Play storefront locations. No parameters.
...
List supported Google Play languages. No parameters.
