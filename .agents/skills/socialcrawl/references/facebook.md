...
**Credit costs:** 12 standard (1 credit), 4 advanced (5), 2 premium (10), 6 custom (flat/metered) - the exact cost is in each endpoint heading below.
...
## GET /v1/facebook/events - 1-13 credits (request-shaped)

List Facebook events for a city

**Cost** 1-13 credits (request-shaped) · **Cache** 600s (a hit costs 0 credits) · **Returns** PostList · **Pagination** cursor - `cursor`
**Pricing** 1 credit for the page. include=details holds 1 credit per event (at most 12 a page) and keeps only the rows filled from a fresh lookup, so a page is at most 13 credits; rows served from cache are free, unfilled rows are refunded, and a repeat of the same call within the cache window costs 0. (metered, 1-13 credits; the response `credits_used` is the real charge after refund).
...
## GET /v1/facebook/profile/events - 1-9 credits (request-shaped)

List a Facebook page's events

**Cost** 1-9 credits (request-shaped) · **Cache** 600s (a hit costs 0 credits) · **Returns** PostList · **Pagination** cursor - `cursor`
**Pricing** 1 credit for the page. include=details holds 1 credit per event (8 a page) and keeps only the rows filled from a fresh lookup, so a page is at most 9 credits; rows served from cache are free, unfilled rows are refunded, and a repeat of the same call within the cache window costs 0. (metered, 1-9 credits; the response `credits_used` is the real charge after refund).
...
- `posts` (optional, integer) - How many of the fetched recent posts to return and compute the metrics over (1-100, default 25). One call reads one page, which on Facebook is 3 posts, so a larger value returns that page and a `_warnings` note; pass posts_cursor back as cursor for the next page. · e.g. `25`
...
## GET /v1/facebook/profile/photos - 1-9 credits (request-shaped)

List Facebook profile photos

**Cost** 1-9 credits (request-shaped) · **Cache** 600s (a hit costs 0 credits) · **Returns** PostList · **Pagination** cursor - `next_page_id`
**Pricing** 1 credit for the page. include=details holds 1 credit per photo (8 a page) and keeps only the rows filled from a fresh lookup, so a page is at most 9 credits; rows served from cache are free, unfilled rows are refunded, and a repeat of the same call within the cache window costs 0. (metered, 1-9 credits; the response `credits_used` is the real charge after refund).
...
## GET /v1/facebook/profile/posts - 1-4 credits (request-shaped)

List Facebook page posts

**Cost** 1-4 credits (request-shaped) · **Cache** 600s (a hit costs 0 credits) · **Returns** PostList · **Pagination** cursor - `cursor`
**Pricing** 1 credit for the page. include=engagement holds 1 credit per row (3 rows a page) and keeps only the rows filled from a fresh lookup, so a page is at most 4 credits; rows served from cache are free, unfilled rows are refunded, and a repeat of the same call within the cache window costs 0. (metered, 1-4 credits; the response `credits_used` is the real charge after refund).
**Reliability** Multi-source: a primary provider with an automatic fallback. You are charged once no matter how many sources are tried.
