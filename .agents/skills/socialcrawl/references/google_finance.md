...
Credit costs on this platform: `quote` is 5 credits (advanced); `ticker-search` and `markets` are 1 credit (standard) — exact cost listed per endpoint below.

Notes:
- Returns the canonical `Quote` archetype — a unified superset across instrument classes (stocks / ETFs / indices / crypto / forex / futures) with a `quote.type` discriminator and a nullable `quote.pair{}` for forex/crypto asset pairs.
- DataForSEO-backed and task-polled upstream — expect ~10–45s latency (use a 60s timeout).
- Resolve an instrument with `ticker-search` first, then pass the returned `id` (e.g. `GOOGL:NASDAQ`) to `quote`.
