...
**Credit costs:** 6 standard (1 credit), 2 advanced (5) - the exact cost is in each endpoint heading below.

<!-- BEGIN:GENERATED:ENDPOINTS (auto-generated by generate-docs.ts - AIP-16b) -->

## GET /v1/amazon/best-sellers - 1 credit (standard)
...
```bash
curl "https://www.socialcrawl.dev/v1/amazon/best-sellers?category=software" \
  -H "x-api-key: $SOCIALCRAWL_API_KEY"
```

## GET /v1/amazon/deals - 1 credit (standard)

Get current Amazon deals

**Cost** 1 credit (standard) · **Cache** 120s (a hit costs 0 credits) · **Returns** ProductList · **Pagination** single page - The deals feed returns one window of current deals with no page parameter on this source.
...
```bash
curl "https://www.socialcrawl.dev/v1/amazon/deals" \
  -H "x-api-key: $SOCIALCRAWL_API_KEY"
```
...
**Cost** 5 credits (advanced) · **Cache** 300s (a hit costs 0 credits) · **Returns** ReviewList · **Pagination** single page - Both sources return one fixed page of on-page reviews and expose no honest pagination: /amazon/product-reviews repeats the same 8 review ids on page 1, 2 and 50 (measured 2026-08-29), and the DFS fallback controls page size by depth in a single call.
**Reliability** Multi-source: a primary provider with an automatic fallback. You are charged once no matter how many sources are tried.
...
## GET /v1/amazon/seller - 1 credit (standard)

Get an Amazon seller profile

**Cost** 1 credit (standard) · **Cache** 900s (a hit costs 0 credits) · **Returns** Seller · **Pagination** none
...
- `seller_id` (required) - Amazon seller id, the seller= value on a seller profile URL. · e.g. `A294P4X9EWVXLJ`
- `country` (optional, enum: US | GB | CA | DE | FR | IT | ES | JP | IN | MX | BR | AU | NL) - Amazon marketplace as an ISO 3166-1 alpha-2 country code (default US). · e.g. `US`

```bash
curl "https://www.socialcrawl.dev/v1/amazon/seller?seller_id=A294P4X9EWVXLJ" \
  -H "x-api-key: $SOCIALCRAWL_API_KEY"
```
