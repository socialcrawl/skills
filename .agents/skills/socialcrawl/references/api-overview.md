...
Keep the key in `SOCIALCRAWL_API_KEY` or `~/.config/socialcrawl/api_key`; never print it, paste it into chat, commit it, put it in a URL, or replace the variable in a shared command with the literal secret. Load the config file into the environment without displaying its contents. If a key is exposed, revoke it at https://socialcrawl.dev/dashboard and create a replacement.
...
- Correct authentication, credit, method, payload, endpoint, and validation errors before retrying.
- Treat `RESOURCE_NOT_FOUND` as a terminal empty result.
- Honour `Retry-After` for rate, concurrency, and service-availability responses; use exponential backoff with jitter.
- Retry a 500 or 502 at most once. For retryable paid non-streaming calls, attach an `Idempotency-Key` before the first attempt and reuse it only for the identical payload.
- Do not automatically retry streaming responses. Report the endpoint, error code, and `request_id`, but never request headers or credentials.
