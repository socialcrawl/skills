...
29 endpoints (26 GET, 3 POST). Base URL `https://www.socialcrawl.dev`, auth header `x-api-key: $SOCIALCRAWL_API_KEY`.

**Credit costs:** 10 standard (1 credit), 2 advanced (5), 1 premium (10), 16 custom (flat/metered) - the exact cost is in each endpoint heading below.
...
## GET /v1/youtube/channel/about - 25 credits (custom)

Get a YouTube channel's contact email and country. Try the 1-credit youtube/channel first: it already carries the email for some channels, and you are charged here only when an address is returned

**Cost** 25 credits (custom) · **Cache** 900s (a hit costs 0 credits) · **Returns** Author · **Pagination** none
**Reliability** Multi-source: a primary provider with an automatic fallback. You are charged once no matter how many sources are tried.
...
- `channelId` (optional, string) - YouTube channel ID. Can pass a channelId, handle or url
- `handle` (optional, string) - YouTube channel handle without the @ symbol · e.g. `mkbhd`
- `url` (optional, string) - YouTube channel URL. Can pass a channelId, handle or url
...
```bash
curl "https://www.socialcrawl.dev/v1/youtube/channel/about?handle=mkbhd" \
  -H "x-api-key: $SOCIALCRAWL_API_KEY"
```
...
## GET /v1/youtube/channel/lives - 1-7 credits (request-shaped)

List a YouTube channel's live streams

**Cost** 1-7 credits (request-shaped) · **Cache** 600s (a hit costs 0 credits) · **Returns** PostList · **Pagination** cursor - `continuationToken`
**Pricing** 1 credit for the page. include=engagement adds 1 credit per distinct stream filled, at most 5 per page; include=channel adds 1 credit for the single channel lookup the page shares. Anything that could not be filled is refunded, so a page with both is at most 7 credits, and a repeat of the same call within the cache window costs 0. (metered, 1-7 credits; the response `credits_used` is the real charge after refund).
...
- Provide at least one of `channelId`, `handle`.
- `include`: at most 2 comma-separated values; each value one of engagement | channel.
...
## GET /v1/youtube/channel/shorts - 1-2 credits (request-shaped)

List YouTube channel shorts

**Cost** 1-2 credits (request-shaped) · **Cache** 600s (a hit costs 0 credits) · **Returns** PostList · **Pagination** cursor - `continuationToken`
**Pricing** 1 credit for the page. include=channel adds 1 credit for the single channel lookup the page shares, refunded if the channel could not be read and free when it was looked up recently. (metered, 1-2 credits; the response `credits_used` is the real charge after refund).
...
## GET /v1/youtube/channel/videos - 1-2 credits (request-shaped)
...
## POST /v1/youtube/channels - 5-100 credits (request-shaped)

Batch get YouTube channel details (up to 1000)

**Cost** 5-100 credits (request-shaped) · **Cache** not cached (every call is live and billed) · **Returns** AuthorList · **Pagination** single page - Batch-by-ids: caller supplies the full id set; there is no next page.
**Pricing** 5 credits per 50-id chunk, so a 50-id batch is 5 credits and the 1,000-id maximum is 100. Batching is what makes this cheap: the same 1,000 channels fetched one at a time through GET /v1/youtube/channel cost 1,000 credits. A chunk is charged whether or not every id in it resolves, so there is no per-id refund; a call that resolves nothing at all is refunded in full (metered, 5-100 credits; the response `credits_used` is the real charge after refund).
...
## GET /v1/youtube/playlist - 1-11 credits (request-shaped)

Get YouTube playlist

**Cost** 1-11 credits (request-shaped) · **Cache** 600s (a hit costs 0 credits) · **Returns** PostList · **Pagination** cursor - `cursor`
**Pricing** 1 credit for the page. include=engagement and include=channel each add 1 credit per distinct video or channel filled, at most 5 per join on a page of up to 50 rows, so a page with both is at most 11 credits; ids that could not be filled are refunded, and a repeat of the same call within the cache window costs 0. (metered, 1-11 credits; the response `credits_used` is the real charge after refund).
**Reliability** Multi-source: a primary provider with an automatic fallback. You are charged once no matter how many sources are tried.
...
## GET /v1/youtube/playlist/items - 1-11 credits (request-shaped)
...
- `handle` (optional, string) - YouTube username or handle, with or without a leading @. One of the identity params is required. · e.g. `mrbeast`
- `channelId` (optional, string) - YouTube channel id, the 24-character string starting with UC. One of the identity params is required.
- `url` (optional, string) - Full YouTube profile or page URL. One of the identity params is required.
- `posts` (optional, integer) - How many of the fetched recent posts to return and compute the metrics over (1-100, default 25). One call reads one page, which on YouTube is up to 30 videos, so a larger value returns that page and a `_warnings` note; pass posts_cursor back as cursor for the next page. · e.g. `25`
...
```bash
curl "https://www.socialcrawl.dev/v1/youtube/profile/full?handle=mrbeast" \
  -H "x-api-key: $SOCIALCRAWL_API_KEY"
```

## GET /v1/youtube/search - 1-11 credits (request-shaped)

Search YouTube

**Cost** 1-11 credits (request-shaped) · **Cache** 120s (a hit costs 0 credits) · **Returns** PostList · **Pagination** cursor - `continuationToken`
**Pricing** 1 credit for the page. include=engagement and include=channel each add 1 credit per distinct video or channel filled, at most 5 per join on a page of up to 50 rows, so a page with both is at most 11 credits; ids that could not be filled are refunded, and a repeat of the same call within the cache window costs 0. (metered, 1-11 credits; the response `credits_used` is the real charge after refund).
...
## GET /v1/youtube/search/advanced - 1-11 credits (request-shaped)

Advanced YouTube video search

**Cost** 1-11 credits (request-shaped) · **Cache** 120s (a hit costs 0 credits) · **Returns** PostList · **Pagination** cursor - `cursor`
**Pricing** 1 credit for the search page. `includeExtras=true` adds a flat 5 credits for the single hydration call that fills view, like and comment counts and `duration_seconds` on the whole page. The increment never repeats: the page caps at 50 results, which one hydration call covers, so a hydrated page of 50 costs 6 credits and so does a hydrated page of 5. include=channel adds 1 credit per distinct channel filled, at most 5 per page, with channels that could not be read refunded, so a page with both is at most 11 (metered, 1-11 credits; the response `credits_used` is the real charge after refund).
...
## GET /v1/youtube/search/hashtag - 1-11 credits (request-shaped)
...
## GET /v1/youtube/shorts/trending - 5-15 credits (request-shaped)

Get trending YouTube shorts

**Cost** 5-15 credits (request-shaped) · **Cache** 120s (a hit costs 0 credits) · **Returns** PostList · **Pagination** single page - Fixed-window feed: upstream returns a single non-cursored result set.
**Pricing** 5 credits for the feed. include=channel adds 1 credit per distinct channel filled, at most 5 per 50 channels (the feed carries about 70, so at most 10); channels that could not be read are refunded, and a repeat of the same call within the cache window costs 0. (metered, 5-15 credits; the response `credits_used` is the real charge after refund).
...
**Pricing** 3 credits per successful transcript. Up to 100 ids per call: the call holds ids × 3 up front (a full batch holds 300) and refunds every not_found, errored and deferred row (metered, 3-300 credits; the response `credits_used` is the real charge after refund).
...
**Cost** 5 credits (advanced) · **Cache** not cached (every call is live and billed) · **Returns** MediaList · **Pagination** single page - Media-file list: every available file of this one kind for the video in a single call; there is no second page. The other three kinds (audio, video files, subtitles, thumbnails) each have their own endpoint.
...
- `continuationToken` (required) - Where to start. Three values work: a top-level comment id (e.g. the `comment.id` of any row from /v1/youtube/video/comments), that row's `comment.ext.replies_token`, or the `pagination.next_cursor` from a previous replies response. A token that is none of these is rejected. · e.g. `Ugzge340dBgB75hWBm54AaABAg`
- `format` (optional, enum: html | plainText) - Text format for reply bodies: 'html' (default) or 'plainText'.

```bash
curl "https://www.socialcrawl.dev/v1/youtube/video/comment/replies?continuationToken=Ugzge340dBgB75hWBm54AaABAg" \
  -H "x-api-key: $SOCIALCRAWL_API_KEY"
```
...
**Cost** 1 credit (standard) · **Cache** not cached (every call is live and billed) · **Returns** MediaList · **Pagination** single page - Media-file list: every available file of this one kind for the video in a single call; there is no second page. The other three kinds (audio, video files, subtitles, thumbnails) each have their own endpoint.
**Empty results** An upstream 404 means "zero items", not "not found" - you get `200 {items: []}` and a refund.
...
## POST /v1/youtube/videos - 5-100 credits (request-shaped)

Batch get YouTube video details (up to 1000)

**Cost** 5-100 credits (request-shaped) · **Cache** not cached (every call is live and billed) · **Returns** PostList · **Pagination** single page - Batch-by-ids: caller supplies the full id set; there is no next page.
...
## GET /v1/youtube/videos/trending - 1-6 credits (request-shaped)

Get trending YouTube videos

**Cost** 1-6 credits (request-shaped) · **Cache** 120s (a hit costs 0 credits) · **Returns** PostList · **Pagination** cursor - `cursor`
**Pricing** 1 credit for the page. include=channel adds 1 credit per distinct channel filled, at most 5 per 50 channels; channels that could not be read are refunded, and a repeat of the same call within the cache window costs 0. (metered, 1-6 credits; the response `credits_used` is the real charge after refund).
