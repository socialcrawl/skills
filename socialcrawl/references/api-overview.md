...
**65 platforms, 575 active endpoints** (31 of them social). Most are `GET /v1/{platform}/{resource}`, but not all: the batch read endpoints take a JSON body over POST, and the stateful `/v1/web/*` job, monitor, and session routes use POST/GET/PATCH/DELETE. Three surfaces sit on top of the registry: the cross-platform **Prism** composites (`/v1/prism/*` and per-platform `profile/full` / `omni-search`), the stateful **Monitors** family (`/v1/monitors/*`), and the stateful **Cohorts** family (`/v1/cohorts/*` + `/v1/cohort-queries/*`) - neither stateful family is counted in the 575. One more call fans out across 17 sources at once: `/v1/search/everywhere`. See [prism.md](prism.md), [monitors.md](monitors.md), [cohorts.md](cohorts.md), and [search.md](search.md).
...
Counted by underlying tier (custom endpoints folded into their base tier), the split is **329 standard - 211 advanced - 35 premium = 575**. Per-endpoint costs, cache TTLs, and the full rule behind every metered endpoint: see [pricing.md](pricing.md).
