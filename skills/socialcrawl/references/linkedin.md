...
**Credit costs:** 3 standard (1 credit), 34 advanced (5), 3 premium (10), 5 custom (flat/metered) - the exact cost is in each endpoint heading below.
...
## GET /v1/linkedin/company/people - 10-50 credits (request-shaped)

List people at a LinkedIn company

**Cost** 10-50 credits (request-shaped) · **Cache** 900s (a hit costs 0 credits) · **Returns** AuthorList · **Pagination** page - `page`, page size `limit`
**Pricing** 10 credits for the list. include=profile holds 4 credits per row and keeps only the rows filled from a fresh lookup: at most 50 for the 10-row page, or 10 + 4 x limit when you pass `limit`. Rows served from cache are free, unfilled rows are refunded, and a repeat of a fully joined page within the list's cache window costs 0. The list is cached for 15 minutes and each profile lookup for 15 minutes. (metered, 10-50 credits; the response `credits_used` is the real charge after refund).
...
## GET /v1/linkedin/post/reactions - 10-50 credits (request-shaped)

List reactors on a LinkedIn post

**Cost** 10-50 credits (request-shaped) · **Cache** 300s (a hit costs 0 credits) · **Returns** AuthorList · **Pagination** page - `page`, page size `limit`
**Pricing** 10 credits for the list. include=profile holds 4 credits per row and keeps only the rows filled from a fresh lookup: at most 50 for the 10-row page, or 10 + 4 x limit when you pass `limit`. Rows served from cache are free, unfilled rows are refunded, and a repeat of a fully joined page within the list's cache window costs 0. The list is cached for 5 minutes and each profile lookup for 15 minutes. (metered, 10-50 credits; the response `credits_used` is the real charge after refund).
...
- `posts` (optional, integer) - How many of the fetched recent posts to return and compute the metrics over (1-100, default 25). One call reads one page, which on LinkedIn company is 10 posts, so a larger value returns that page and a `_warnings` note; pass posts_cursor back as cursor for the next page. · e.g. `25`
...
- `easy_apply` (optional, boolean) - When true, return only jobs that support LinkedIn Easy Apply.
- `sort_by` (optional, enum: recent | relevant) - Result ordering.
- `company` (optional, string) - Filter by company, using a numeric company id from /v1/linkedin/company (`author.id`). · e.g. `1035`
- `geocode` (optional, string) - Filter by location, using a geocode id from /v1/linkedin/search/location. · e.g. `103644278`
- `industry_ids` (optional, string) - Comma-separated industry ids from /v1/linkedin/search/industry. · e.g. `4`
...
## GET /v1/linkedin/search/people - 10-50 credits (request-shaped)

Search LinkedIn people

**Cost** 10-50 credits (request-shaped) · **Cache** 120s (a hit costs 0 credits) · **Returns** AuthorList · **Pagination** page - `page`, page size `limit`
**Pricing** 10 credits for the list. include=profile holds 4 credits per row and keeps only the rows filled from a fresh lookup: at most 50 for the 10-row page, or 10 + 4 x limit when you pass `limit`. Rows served from cache are free, unfilled rows are refunded, and a repeat of a fully joined page within the list's cache window costs 0. The list is cached for 2 minutes and each profile lookup for 15 minutes. (metered, 10-50 credits; the response `credits_used` is the real charge after refund).
**Reliability** Multi-source: a primary provider with an automatic fallback. You are charged once no matter how many sources are tried.
...
- `current_company` (optional, string) - Filter by current company, using a numeric company id from /v1/linkedin/company (`author.id`). Comma-separated for multiple. · e.g. `1035`
- `past_company` (optional, string) - Filter by a previously-worked company, using a numeric company id from /v1/linkedin/company (`author.id`). · e.g. `1035`
- `school` (optional, string) - Filter by school, using a school id from /v1/linkedin/search/schools (`id`). · e.g. `1792`
- `industry` (optional, string) - Filter by industry, using an industry id from /v1/linkedin/search/industry (`industry_id`). · e.g. `4`
- `geocode_location` (optional, string) - Filter by location, using a geocode id from /v1/linkedin/search/location (`geocode`). · e.g. `103644278`
- `profile_language` (optional, string) - Filter by profile language (ISO 2-letter code, e.g. 'en').
- `service_category` (optional, string) - Filter by service category, using a numeric service-category id. No endpoint resolves these, so pass one you already hold. · e.g. `1`
- `follower_of` (optional, string) - Return people who follow a specific member, using the member's URN from /v1/linkedin/profile (`author.ext.urn`). This one is a URN and not a numeric id. · e.g. `ACoAAA8BYqEBCGLg_vT_ca6mMEqkpp9nVffJ3hc`
- `include` (optional, enum: profile) - Set to `profile` (one token only) to join every row to the member's profile lookup in this one call: `author.followers` becomes the exact count and `author.ext.followers_approximate` reads false on every row the lookup filled, `author.following` is filled with the member's connection count, and the location, joined date, country, website, cover image and profile flags land where the row lacks them. Holds 4 credits per row and keeps only the rows filled from a fresh lookup; rows already in cache and rows that could not be filled are refunded, so a call costs at most 50 (10 + 4 per row) and the usual bill is lower. Adds about 1 second for 10 rows, up to 3 on a slow second and never more than 8, and nothing when the members are already cached; read `data.hydration` for the rows, the credits held and kept, and the time.
- `limit` (optional, integer, 1-10) - Take the top N rows of this page (1 to 10). With `include=profile` it caps both the rows joined and the extra credits: `limit=3&include=profile` costs at most 22. The page cursor still advances past the full page of 10, so the rows past your limit are skipped, not carried to the next page. Without `include=profile` the call stays 10 credits.
...
**Cost** 5 credits (advanced) · **Cache** 120s (a hit costs 0 credits) · **Returns** PostList · **Pagination** page - `page`
**Reliability** Multi-source: a primary provider with an automatic fallback. You are charged once no matter how many sources are tried.
...
- `from_company` (optional, string) - Filter to posts from a company, by numeric LinkedIn company id (for example `1035`). A company slug or URL is rejected; read the id from `author.id` on /v1/linkedin/company.
