...
- `brand` (required) - The brand whose appearance-% is measured (required). Matched against each answer plus its aliases. · e.g. `socialcrawl`
...
- `competitors` (optional, string) - CSV of up to 5 competitors also measured for appearance-% from the same answers. · e.g. `competitor one,competitor two`
...
- Provide at least one of `topic`, `prompts`.

```bash
curl "https://www.socialcrawl.dev/v1/prism/ai-visibility?brand=socialcrawl&topic=social media data api" \
...
- `country` (optional, string) - Storefront country, as a location name or numeric code (look one up with `GET /v1/app_store/locations` or `GET /v1/google_play/locations`). Defaults to United States.
...
```bash
curl "https://www.socialcrawl.dev/v1/prism/app-reviews?query=spotify" \
  -H "x-api-key: $SOCIALCRAWL_API_KEY"
```
...
- `include` (optional, string) - CSV subset of `merged`, `similar`, `developer` (default: all three). Naming a subset TRIMS which blocks are computed; an unknown member or an empty value is a 400.
...
**Pricing** 2 credits per found TikTok item and 5 per found Instagram item, raised by `deep_scan`. Up to 25 items per call: the whole batch holds at most 100 credits up front and refunds every not_found, errored and deferred item (metered, 2-100 credits; the response `credits_used` is the real charge after refund).
**Streaming** Optional SSE - send `Accept: text/event-stream` for incremental frames, or omit it for one JSON body.
...
## GET /v1/prism/comments - 2-200 credits (request-shaped)

Every comment on a post, replies nested, server-paginated to completion.

**Cost** 2-200 credits (request-shaped) · **Cache** 300s (a hit costs 0 credits) · **Returns** Analytics · **Pagination** cursor - `cursor`, page size `limit`
...
## GET /v1/prism/creator-card - 5-8 credits (request-shaped)

One handle, unified author cards across TikTok, Instagram, YouTube, X (and more).

**Cost** 5-8 credits (request-shaped) · **Cache** 900s (a hit costs 0 credits) · **Returns** Analytics · **Pagination** none
**Pricing** 5 credits covering any 4 platforms, plus 1 credit per platform beyond 4. The default selection is 4 platforms, so the 7-platform maximum is 8 credits (metered, 5-8 credits; the response `credits_used` is the real charge after refund).
...
## GET /v1/prism/creator-vet - 50-75 credits (request-shaped)
...
**Cost** 50-75 credits (request-shaped) · **Cache** 1800s (a hit costs 0 credits) · **Returns** Analytics · **Pagination** none
**Pricing** 50 credits for the standard vet. `include=cross_platform` raises it to 75 for the cross-platform identity check on top (metered, 50-75 credits; the response `credits_used` is the real charge after refund).
...
## GET /v1/prism/crisis-radar - 15-45 credits (request-shaped)
...
**Cost** 15-45 credits (request-shaped) · **Cache** 1800s (a hit costs 0 credits) · **Returns** Analytics · **Pagination** none
**Pricing** 15 credits for the baseline breach check. `confirm=true` escalates to the full 45-credit investigation. Run the baseline first and escalate only when it reports a breach: that is the whole point of the two-step shape (metered, 15-45 credits; the response `credits_used` is the real charge after refund).
...
- `brand` (required) - The brand to watch. · e.g. `socialcrawl`
...
```bash
curl "https://www.socialcrawl.dev/v1/prism/crisis-radar?brand=socialcrawl" \
  -H "x-api-key: $SOCIALCRAWL_API_KEY"
```
...
Fans out content_analysis news search, a Hacker News leg, and a fresh-web news leg in parallel, then dedupes and ranks them into one earned-media clip feed with computed top outlets, freshness labeling (content_analysis fetch_time lags the live web), and (when a `competitor` is supplied) a coverage gap. The content_analysis news leg is the coverage spine (losing it → 50% refund); Hacker News and the fresh-web leg are enrichment. `include=digest` adds an LLM narrative summary leg.
...
## GET /v1/prism/handle-audit - 5-8 credits (request-shaped)
...
**Cost** 5-8 credits (request-shaped) · **Cache** 1800s (a hit costs 0 credits) · **Returns** Analytics · **Pagination** none
...
## GET /v1/prism/korea-gap - 15-40 credits (request-shaped)
...
**Cost** 15-40 credits (request-shaped) · **Cache** 1800s (a hit costs 0 credits) · **Returns** Analytics · **Pagination** none
**Pricing** 15 credits for the web-only read. The default `include` carries the social leg, which makes the call 40; pass an `include` without `social` to stay at 15 (metered, 15-40 credits; the response `credits_used` is the real charge after refund).
...
- `include` (optional, string) - CSV subset of `social` (the cross-platform everywhere leg) and `quotes` (the LLM translation pass), default both. An unknown member is a 400. Note the price runs the OTHER way from `video-intel` and `creator-vet`: DROPPING `social` moves the call from 40 credits to the 15-credit web-only variant. · e.g. `social,quotes`
- `date_from` (optional, string) - Window start (YYYY-MM-DD).
- `date_to` (optional, string) - Window end (YYYY-MM-DD).
...
Universal post/product URL dispatcher: any post, video, product or repo link → the right detail endpoint's unified response.

**Cost** free (0 credits) · **Cache** 600s (a hit costs 0 credits) · **Returns** Analytics · **Pagination** none
**Pricing** the resolved endpoint's own price, with no routing surcharge: 1 credit for most social lookups and 5 for the advanced ones (Amazon, LinkedIn). The `resolved.endpoint` field in the response names the endpoint that was charged.

Sniffs the platform + endpoint from a URL and returns that endpoint's unified data, with no surcharge. You pay the resolved endpoint's own cost. The join key the whole API runs on is `url`. Takes an ITEM link (a post, video, pin, tweet, repo, product or track), not a profile or channel link, on TikTok, Instagram, YouTube, X/Twitter, Facebook, LinkedIn, Bluesky, Pinterest, Reddit, Threads, Rumble, Truth Social, GitHub, Amazon and Spotify. The `resolved` block names the platform, endpoint, and archetype of the response so agents know the shape that follows; `legs[]` reports the dispatched call's status, cost, and latency.
...
## GET /v1/prism/org-radar - 6-51 credits (request-shaped)
...
**Cost** 6-51 credits (request-shaped) · **Cache** 1800s (a hit costs 0 credits) · **Returns** Analytics · **Pagination** none
**Pricing** 1 credit for the org read plus 5 per repository expanded into a dossier. The default of 5 repos is 26 credits, a single repo is 6, and the 10-repo maximum is 51. Set `repos` to bound it before calling (metered, 6-51 credits; the response `credits_used` is the real charge after refund).
...
- `include` (optional, string) - CSV subset of `dossiers`, `rollup` (default: both). Naming a subset TRIMS which blocks are computed; an unknown member or an empty value is a 400. Dropping `dossiers` also drops the per-repo credit (`1 + 5 x repos` becomes `1`).
...
**Pricing** 1 credit per successful URL on most platforms, 5 on Instagram and LinkedIn, charged at each URL's own platform rate. Up to 100 URLs per call: the call holds the summed worst case up front (a 100-URL Instagram batch holds 500) and refunds every dead, errored and unsupported URL, so you pay for exactly the rows that returned counts (metered, 1-500 credits; the response `credits_used` is the real charge after refund).
**Streaming** Optional SSE - send `Accept: text/event-stream` for incremental frames, or omit it for one JSON body.
...
- `country` (optional, string) - Marketplace country, as a location name or numeric code (look one up with `GET /v1/content_analysis/locations`).
...
- `include` (optional, string) - CSV subset of `topics`, `timeline`, `sellers`, `retailer_matrix`, `social_echo` (default: all five). Naming a subset TRIMS which blocks are computed; an unknown member or an empty value is a 400. `sellers` also needs `amazon` to be in scope.
...
- `country` (optional, string) - Marketplace or locale, as a location name or numeric code (look one up with `GET /v1/content_analysis/locations`). Defaults to United States.
- `depth` (optional, integer) - Reviews per source (Trustpilot clamped ≤20).
...
- `include` (optional, string) - CSV subset of `themes`, `response_rate`, `languages` (default: all three). Naming a subset TRIMS the report to those blocks; an unknown member or an empty value is a 400.
...
- `country` (optional, string) - Marketplace country, as a location name or numeric code (look one up with `GET /v1/content_analysis/locations`). Defaults to United States.
...
## GET /v1/prism/share-of-voice - 20-200 credits (request-shaped)

Engagement-weighted Share of Voice across 2-5 brands, with web+social split, emotion overlay, and ESOV.

**Cost** 20-200 credits (request-shaped) · **Cache** 1800s (a hit costs 0 credits) · **Returns** Analytics · **Pagination** none
**Pricing** 40 credits per brand with the social leg, which is the default, or 20 per brand for the web-only read. Up to 5 brands per call, so the 200-credit ceiling is 5 brands with social and dropping `social` from `include` halves it. Price the call as `brands x per-brand` before you send it - this is the most expensive composite on the API (metered, 20-200 credits; the response `credits_used` is the real charge after refund).
...
- `category_code` (optional, string) - Numeric category-taxonomy code for true-share-of-category (look one up with `GET /v1/content_analysis/categories`).
...
- `include` (optional, string) - CSV toggles (default `emotions,social`). Drop `social` for the cheaper web-only variant; drop `emotions` to skip the sentiment leg. The emotion overlay is third-party NLP passed through unmodified and is not scoped to your date window; `methodology.sentiment_provenance` states that on every response that carries it. · e.g. `social,emotions`
...
## GET /v1/prism/video-intel - 5-15 credits (request-shaped)

One video URL → detail + stats + transcript + top comments + commenter sample, across YouTube/TikTok/Rumble/Instagram.

**Cost** 5-15 credits (request-shaped) · **Cache** 600s (a hit costs 0 credits) · **Returns** Analytics · **Pagination** none
**Pricing** 5 credits for the video detail, stats and comments. `include=transcript` adds 10, which is exactly what the standalone transcript endpoint charges, so the bundle never costs more than running the two calls yourself (metered, 5-15 credits; the response `credits_used` is the real charge after refund).
